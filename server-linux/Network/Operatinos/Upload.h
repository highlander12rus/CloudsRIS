...
namespace Network {
    namespace Tcp {
        namespace Operatinos {
             ...
             * @param buffer
             */
            class Upload : public boost::enable_shared_from_this<Upload> /*public TcpOperation*/ {
            public:
                typedef boost::shared_ptr<Upload> pointer;
                ...
                /**
                 * Оперция выгрузки в заголовке протокола
                 */
                static char const OPERATION_UPLOADING;
                ...
                Upload(boost::asio::io_service* io_service, Config* config,
                ...
                /**
                 * Выполняет отсылку файлов на N серверов
                 * @param data массив котоырй отправить
                 * @param length длинна ссобщения
                 */
                virtual void BackupSendData(char* data, uint_32 length);

                /**
                 * Выполняет поиск сереров для бекапа передн началом приемки файла
                 */
                virtual void SearchBackUpServers();
                 ...
                 * @return 
                 */
                bool isAccessOperationCode(char type);
